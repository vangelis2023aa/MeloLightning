...
void MetalSynchronizedHeapAllocator::FreeReservation(AllocatorReservation* uploadReservation)
{
	...
	// the heap allocation handle is stored on the reservation at alloc time, so freeing is O(1)
	// (no linear scan of a live-allocation list, which was O(n) per free / O(n^2) per busy frame)
